...
total = duration(src)
long_sil = [s for s in silences(src, -35, 0.6) if s[0] > 0.05 and s[1] < total - 0.05]
if len(long_sil) != len(chunks) - 1:
    sys.exit(f"expected {len(chunks) - 1} long pauses, found {len(long_sil)}: {long_sil}")
